...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockViewModel.addRuleScript(any(), enabled: any(named: 'enabled'))).thenAnswer((_) async {});
  ...
  Widget createWidgetUnderTest() {
    return MaterialApp(home: ScriptScreenWidget(viewModel: mockViewModel));
  }
  ...
  testWidgets('Edit script dialog does not have a name field', (WidgetTester tester) async {
    ...
    verify(
      () => mockViewModel.addRuleScript('define UpdatedScript for roll *d*\n action blink', enabled: true),
    ).called(1);
  });
